...
export const Wish = memo(({ modalIsOpen, setIsOpen }: ModalIsOpenType) => {
  ...
  const schema = yup.object({
    title: yup
      ...
      .max(10, "Max value, put 10 symbols or less!"),
    price: yup
      .number()
      .typeError("Field is required as a number!")
      .required(),
    links: yup
      .string()
      .required("Field is required!"),
    description: yup
      ...
      .max(220, "Max value, put less < 220!")
  })
  ...
  const onSubmitHandler: SubmitHandler<WishTypeForm> = (data) => {
    ...
    dispatch(createWishThunk({ id, description, title, price, categories, urlLinks }))
    navigate("/wishlist")
    setIsOpen(false)
    reset()
  }
